...
class FtCtrl extends GenderizeioControl
{
...
class FtClient
{
    ...
    private GenderizeioUtility $utility;

    public function __construct(GenderizeioUtility $utility, array $options)
    {
    ...
    public function get_utility(): GenderizeioUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GenderizeioContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GenderizeioContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GenderizeioContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GenderizeioConfig::make_feature($name);
    ...
    private static function buildUrl(GenderizeioSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GenderizeioError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GenderizeioResponse($fetched);
        }
    ...
    private function fail(GenderizeioContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GenderizeioContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
