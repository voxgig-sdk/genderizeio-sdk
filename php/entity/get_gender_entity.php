...
// Genderizeio SDK GetGender entity
...
class GetGenderEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetGenderEntity($this->_client, $opts);
    }

    /**
     * @param GetGender|array $args GetGender data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GenderizeioHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetGender|array The current GetGender data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetGender fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GenderizeioHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetGender fields).
     */
    ...
    /**
     * Load a single GetGender.
     *
     * @param GetGenderLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetGenderLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetGender|array The loaded GetGender as an assoc-array at the
     *   SDK boundary; throws GenderizeioError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = GenderizeioHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
