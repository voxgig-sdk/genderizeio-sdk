...
require_relative 'Genderizeio_types'
...
class GenderizeioSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GenderizeioHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GenderizeioHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GenderizeioFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GenderizeioUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GenderizeioHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GenderizeioHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GenderizeioHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = GenderizeioHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GenderizeioError.new(
        "graphql_error", "GenderizeioSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = GenderizeioSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
