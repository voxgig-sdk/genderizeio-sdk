...
class GetGenderEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = get_gender_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "get_gender." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GENDERIZEIO_TEST_GET_GENDER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_gender_ref01_data = nil
    if get_gender_ref01_data_raw.length > 0
...
def get_gender_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "get_gender", "GetGenderTestData.json")
  ...
  entid_env_raw = ENV["GENDERIZEIO_TEST_GET_GENDER_ENTID"]
  ...
  if env["GENDERIZEIO_TEST_LIVE"] == "TRUE"
    ...
    client = GenderizeioSDK.new(Helpers.to_map(merged_opts))
  end
