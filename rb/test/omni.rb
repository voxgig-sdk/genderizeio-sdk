...
module GenderizeioOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if GenderizeioOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = GenderizeioOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| GenderizeioOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || GenderizeioOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = GenderizeioSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = GenderizeioError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { GenderizeioOmni.errify(err) }
