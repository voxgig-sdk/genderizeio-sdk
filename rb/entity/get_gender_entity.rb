...
class GetGenderEntity
  ...
  def make
    opts = @_entopts.dup
    GetGenderEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GenderizeioHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetGender, Hash] the current GetGender data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GenderizeioHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetGender fields)
  def match_get
  ...
  #
  # @param reqmatch [GetGenderLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetGender.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetGender, Hash] the loaded GetGender; raises GenderizeioError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GenderizeioHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
