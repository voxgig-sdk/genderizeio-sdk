...
class GenderizeioTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GenderizeioTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GenderizeioTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GenderizeioTestRunner.getenv("GENDERIZEIO_TEST_LIVE")
        override = GenderizeioTestRunner.getenv("GENDERIZEIO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GenderizeioTestRunner.getenv(key)
        ...
        explain = GenderizeioTestRunner.getenv("GENDERIZEIO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GENDERIZEIO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GenderizeioTestRunner._test_control is not None:
            return GenderizeioTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GenderizeioTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GenderizeioTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GenderizeioTestRunner.load_test_control()
...
def load_env_local():
    GenderizeioTestRunner.load_env_local()


def env_override(m):
    return GenderizeioTestRunner.env_override(m)


def entity_data(v):
    return GenderizeioTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GenderizeioTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GenderizeioTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GenderizeioTestRunner.load_test_control()


def live_client_options():
    return GenderizeioTestRunner.live_client_options()


def live_delay_ms():
    return GenderizeioTestRunner.live_delay_ms()
