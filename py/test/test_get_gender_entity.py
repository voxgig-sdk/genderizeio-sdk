...
class TestGetGenderEntity:

    def test_should_create_instance(self):
        testsdk = GenderizeioSDK.test(None, None)
        ent = testsdk.GetGender(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _get_gender_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "get_gender." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GENDERIZEIO_TEST_GET_GENDER_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        get_gender_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.get_gender")))
        get_gender_ref01_data = None
        if len(get_gender_ref01_data_raw) > 0:
...
def _get_gender_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/get_gender/GetGenderTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GENDERIZEIO_TEST_GET_GENDER_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("GENDERIZEIO_TEST_LIVE") == "TRUE":
        ...
        client = GenderizeioSDK(helpers.to_map(merged_opts))

    _live = env.get("GENDERIZEIO_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("GENDERIZEIO_TEST_EXPLAIN") == "TRUE",
        "live": _live,
