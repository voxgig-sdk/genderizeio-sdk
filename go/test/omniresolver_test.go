...
import (
	...
	omni "github.com/voxgig-sdk/genderizeio-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GenderizeioError); is {
				out := map[string]any{
					"name":    "GenderizeioError",
...
func omniCtx(arg any, client *sdk.GenderizeioSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GenderizeioSDK); is {
			client = live
