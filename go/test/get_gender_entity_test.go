...
func TestGetGenderEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		...
		ent := testsdk.GetGender(nil)
		if ent == nil {
			t.Fatal("expected non-nil GetGenderEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := get_genderBasicSetup(nil)
		...
		for _, _op := range []string{"load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "get_gender." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		_ = getGenderRef01Data

		// LOAD
		getGenderRef01Ent := client.GetGender(nil)
		getGenderRef01MatchDt0 := map[string]any{}
		getGenderRef01DataDt0Loaded, err := getGenderRef01Ent.Load(getGenderRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if getGenderRef01DataDt0Loaded == nil {
...
func get_genderBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "get_gender", "GetGenderTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read get_gender test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse get_gender test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("GENDERIZEIO_TEST_GET_GENDER_ENTID")
