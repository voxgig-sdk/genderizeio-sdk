...
[MCP](https://modelcontextprotocol.io) server exposing the Genderizeio SDK as
two agent tools — `genderizeio_list` and `genderizeio_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/genderizeio-mcp)
make build

# 2. Provide credentials via the environment
export GENDERIZEIO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user genderizeio \
  -- /absolute/path/to/genderizeio-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./genderizeio-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// genderizeio_list: first page of records
{ "entity": "get_gender" }
{ "entity": "get_gender", "query": { } }

// genderizeio_load: one record by id
{ "entity": "get_gender", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/genderizeio-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user genderizeio \
     -- "$PWD"/dist/*/genderizeio-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `genderizeio_list` and `genderizeio_load` tools now appear
   in new sessions. Ask the agent to *"list get_gender using genderizeio"*
   and it calls `genderizeio_list` with `{"entity":"get_gender"}`.
...
```sh
export GENDERIZEIO_APIKEY=sk_live_xxx            # API key
export GENDERIZEIO_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `genderizeio_list` tool
...
### Call the `genderizeio_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `genderizeio_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `genderizeio_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GENDERIZEIO_APIKEY` | API key sent with every request. |
| `GENDERIZEIO_BASE` | Optional override of the API base URL. |
...
```sh
./genderizeio-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"genderizeio_load","arguments":{"entity":"get_gender","query":{"id":1}}}}'
```
