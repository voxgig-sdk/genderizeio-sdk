...
local GenderizeioSDK = {}
GenderizeioSDK.__index = GenderizeioSDK
...
GenderizeioSDK._make_feature = _make_feature


function GenderizeioSDK.new(options)
  local self = setmetatable({}, GenderizeioSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function GenderizeioSDK:options_map()
...
function GenderizeioSDK:get_utility()
  return Utility.copy(self._utility)
end


function GenderizeioSDK:get_root_ctx()
  return self._rootctx
end


function GenderizeioSDK:prepare(fetchargs)
...
function GenderizeioSDK:direct(fetchargs)
...
-- by passing it.
function GenderizeioSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function GenderizeioSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "GenderizeioSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function GenderizeioSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = GenderizeioSDK.new(sdkopts)
  sdk.mode = "test"
