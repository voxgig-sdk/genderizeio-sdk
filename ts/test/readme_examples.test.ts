...
//     (\`new GenderizeioSDK(...)\` / \`GenderizeioSDK.test(...)\` become
//     \`GenderizeioSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_gender":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GenderizeioSDK(...)\` and any
// \`GenderizeioSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GenderizeioSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GenderizeioSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
