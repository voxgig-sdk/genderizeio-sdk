// Command Genderizeio-cli is a CLI + REPL for the Genderizeio SDK,
...
const prompt = "genderizeio"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_gender"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GENDERIZEIO_APIKEY (api key), GENDERIZEIO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GENDERIZEIO_APIKEY carries the API key and
	// GENDERIZEIO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GENDERIZEIO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GENDERIZEIO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGenderizeioSDK(opts)
