...
boru-driven command-line client **and** interactive REPL for the Genderizeio
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/genderizeio-cli)
make build

# 2. See usage (words, entities, env vars)
./genderizeio-cli --help

# 3. Provide credentials once, via the environment
export GENDERIZEIO_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./genderizeio-cli load 1 get_gender            # {id:1} shorthand
./genderizeio-cli load '{id:1}' get_gender       # explicit match map

# 5. Override the API base URL for a single call
GENDERIZEIO_BASE=https://api.example.com ./genderizeio-cli load 1 get_gender

# 6. No arguments -> interactive REPL
./genderizeio-cli
genderizeio> load 1 get_gender
genderizeio> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/genderizeio-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/genderizeio-cli load 1 get_gender
   ```
...
```sh
./genderizeio-cli load 1 get_gender          # scalar shorthand for {id:1}
./genderizeio-cli load '{id:1}' get_gender     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `genderizeio>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_gender`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/genderizeio-cli`. |
...
(`./genderizeio-cli <expr>`) and interactively (the REPL), and expressions compose the
