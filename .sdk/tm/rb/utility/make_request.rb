...
module GenderizeioUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GenderizeioResponse.new({})
    ...
    if fetch_err
      ...
      response = GenderizeioResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GenderizeioResponse.new(fetched)
    else
