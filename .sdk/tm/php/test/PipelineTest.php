...
{
    private static function utility(): GenderizeioUtility
    {
        return new GenderizeioUtility();
    }
    ...
    private static function ctx(array $over = []): GenderizeioContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GenderizeioContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GenderizeioOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GenderizeioError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GenderizeioOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GenderizeioMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GenderizeioOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GenderizeioMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GenderizeioOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GenderizeioMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GenderizeioSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GenderizeioMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GenderizeioResponse([]),
            'result' => new GenderizeioResult([]),
        ]);
        [, $err] = GenderizeioMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GenderizeioSpec([]),
            'response' => null,
            'result' => new GenderizeioResult([]),
        ]);
        [, $err] = GenderizeioMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GenderizeioSpec(['step' => 's']),
            'response' => new GenderizeioResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GenderizeioResult([]),
        ]);
        [, $err] = GenderizeioMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GenderizeioSpec(['step' => 's']),
            'response' => new GenderizeioResponse(self::resp(200, ['v' => 1])),
            'result' => new GenderizeioResult([]),
        ]);
        [, $err] = GenderizeioMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GenderizeioSpec(['step' => 's']),
            'response' => new GenderizeioResponse(self::resp(200, ['v' => 2])),
            'result' => new GenderizeioResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GenderizeioMakeResponse::call($ctx);
    ...
    {
        $preset = new GenderizeioResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GenderizeioSpec([]),
            'response' => new GenderizeioResponse([]),
            'result' => new GenderizeioResult([]),
        ...
        [$got, $err] = GenderizeioMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GenderizeioResult([])]);
        [, $err] = GenderizeioMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GenderizeioSpec([]), 'result' => null]);
        [, $err] = GenderizeioMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GenderizeioSpec(['step' => 's']),
            'result' => new GenderizeioResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GenderizeioOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GenderizeioMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GenderizeioSpec(['step' => 's']),
            'result' => new GenderizeioResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GenderizeioOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GenderizeioMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GenderizeioResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GenderizeioSpec([]),
            'result' => new GenderizeioResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GenderizeioMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GenderizeioMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GenderizeioError('boom', 'boom');
        $utility->fetcher = function (GenderizeioContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GenderizeioSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GenderizeioMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GenderizeioContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GenderizeioMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GenderizeioResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        GenderizeioMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GenderizeioContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GenderizeioSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GenderizeioMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GenderizeioResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GenderizeioSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GenderizeioMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GenderizeioMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GenderizeioMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GenderizeioResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GenderizeioDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GenderizeioResult(['ok' => false])]);
        $this->expectException(GenderizeioError::class);
        GenderizeioDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GenderizeioResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GenderizeioMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GenderizeioMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GenderizeioBaseFeature();
        $b = new GenderizeioBaseFeature();
        GenderizeioFeatureAdd::call($ctx, $a);
        GenderizeioFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GenderizeioBaseFeature
    {
        $f = new GenderizeioBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GenderizeioFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GenderizeioContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GenderizeioSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GenderizeioPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        GenderizeioPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GenderizeioResponse(['headers' => null]),
            'result' => new GenderizeioResult([]),
        ]);
        GenderizeioResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        GenderizeioResultBody::call($ctx);
